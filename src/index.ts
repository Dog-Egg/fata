...
export function string({
  ...
  if (min < 0 || max < 0) {
    throw new Error("The length of the string cannot be less than 0.");
  }

  const count = integer({ min, max });
  const arr = [];
  for (let i = 0; i < count; i++) {
    arr.push(DICT[integer({ min: 0, max: DICT.length - 1 })]);
  }
  return arr.join("");
}

export function image({
  width = integer({ min: 6, max: 120 }) * 10,
  height = integer({ min: 6, max: 120 }) * 10
